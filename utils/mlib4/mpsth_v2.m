function [psth psth_trials trialspx] = mpsth_v2(spxtimes,trigtimes,varargin)
...
% IMPORTANT: all timing inputs must be seconds and will be converted in the process!
%
% MANDATORY INPUTS
...
if binsz>1
  psth = zeros(pre/binsz+post/binsz,2);         % one extra chan for timebase
...
trialspx = cell(numel(trigtimes),1);
  ...
  psth_trials = zeros(pre+post+1,numel(trigtimes)+1); %%%%%%%%%%%%%%%%%%%%%%%
  psth_trials(:,1) = psth(:,1) ;
...
for i = 1:numel(trigtimes)
  ...
  if binsz==1 % just to make sure...
   ...
   psth_trials(trialspx{i}+pre+1,i+1) = psth_trials(trialspx{i}+pre+1,i+1)+1;    %%%%%%%%%%%%%%%%%%%%%%%
  elseif binsz>1
    try
...
if chart==1
  ...
  xlabel('peri-stimulus time'),ylabel(['counts per ' num2str(binsz) 'ms bin'])
elseif chart==2
  figure('name','peri-stimulus time histogram','units','normalized','position',[0.3 0.3 0.4 0.3])
  subplot(212)
  bar(psth(:,1)+binsz,psth(:,2),'k','BarWidth',1)
  axis([min(psth(:,1))-10 max(psth(:,1))+10 0 max(psth(:,2))+1])
  ylabel(['counts per ' num2str(binsz) 'ms bin'])
  subplot(211)
